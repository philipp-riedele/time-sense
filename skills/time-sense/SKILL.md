---
name: time-sense
description: "(Skill) Activates whenever time, duration, or temporal reasoning is relevant — even implicitly. Core principle: if a question involves change over time, status of a process, ordering of events, or ANY situation where Claude might guess about time instead of checking, this skill applies. Direct triggers: time/duration questions, process status ("is it done?", "is it running?", "did it work?"), file/log analysis, build/deploy monitoring, background tasks, scheduling, transfer speed estimates, timestamp comparisons, git history, file age checks, deadline reasoning, wait/retry decisions. Meta-triggers — activate even WITHOUT explicit time words: state change questions ("has anything changed?", "is this up to date?"), progress/completion ("are we done?", "what's the status?"), comparison to previous state ("it used to work", "it was different before"), frequency ("again?", "still?"), unexpectedly long/short durations ("why is this taking so long?"), event reconstruction from logs/commits/history, and any situation where Claude is tempted to say "recently", "a while ago", "earlier", "soon", "just now", or to suggest waiting, sleeping, polling, or checking back later.
---
